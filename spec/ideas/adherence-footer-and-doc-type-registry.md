...
Today `REQ: adherence-footer` lives in `spec/features/feature/README.md` and hardcodes the URL `https://specscore.md/feature-specification`. The lint checker `pkg/lint/adherence_footer.go` (in [`specscore-cli`](https://github.com/specscore/specscore-cli)) hardcodes the same constant. One exception exists: the recently-extracted `spec/features/plan/plans-index/` sub-feature defines `plans-index-specification`.
...
The lint `pkg/lint/adherence_footer.go` (in [`specscore-cli`](https://github.com/specscore/specscore-cli)) is refactored to read `Kind`, `URL`, and `Consumer Path` from the registry, then walk consumer paths and check the correct footer URL per Kind. Structure and Meta Kinds are not walked.
