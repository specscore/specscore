...
| `Rejected` | Idea was considered and turned down — an explicit decision against it. Reachable from `Draft`, `In Review`, `Approved`, `Specifying`, `Specified`, or `Implementing` — an Idea can be actively turned down at any point before it ships, including while still in `Draft` (the state where an idea is most likely to be killed deliberately). Terminal. The disposition reason is captured as the transition reason. |
...
An Idea reaches a terminal disposition status in one of two ways. With this REQ, the shared disposition vocabulary is **complete** across the entire pre-terminal lifecycle: every non-terminal status can record either an active turn-down (`Rejected`) or passive decay (`Stale`) — no status is left able to say only one of the two.

- **`Rejected`** — an explicit decision against the Idea, reachable from `Draft`, `In Review`, `Approved`, `Specifying`, `Specified`, or `Implementing` (`Draft → Rejected`, `In Review → Rejected`, `Approved → Rejected`, `Specifying → Rejected`, `Specified → Rejected`, `Implementing → Rejected`). The transition is author-driven and is a **negative transition** that MUST carry a reason (recorded as the transition reason — e.g. via `specscore` change-status — not as a header field).
...
`Rejected` and `Stale` are not interchangeable even when both are reachable from the same status: `Rejected` is an **active** decision to turn the Idea down, `Stale` is **passive** decay — nobody decided against it, it simply was never carried forward (see [status-vocabulary#req:parked-stale-vs-withdrawn](../status-vocabulary/README.md#req-parked-stale-vs-withdrawn) for the shared-vocabulary distinction this mirrors). This holds at every stage, including the earliest one: a `Draft` Idea that its author deliberately kills — the state where an idea is most likely to be killed deliberately, since it hasn't been shared for feedback yet — is `Rejected`, not `Stale`; forcing that decision into `Stale` would record it as passive decay, which it is not. The same distinction carries forward through `In Review`, an `Approved` Idea cancelled before specification work starts, a `Specifying` or `Specified` Idea whose author gives up on it, and an `Implementing` Idea whose build is actively called off versus one that simply petered out. `Feature`'s `Draft` already has an active-abandonment exit, spelled `Deprecated` because Feature's disposition set includes it (see [feature#feature-statuses](../feature/README.md#feature-statuses)); `Draft → Rejected` is Idea's equivalent of that same door, spelled with Idea's own disposition vocabulary. [status-vocabulary#req:disposition-vocabulary](../status-vocabulary/README.md#req-disposition-vocabulary)'s requirement that `Rejected` be permitted "from its review... state" is a floor, not a ceiling — nothing there forbids extending it earlier.
