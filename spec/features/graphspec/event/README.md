...
- **Participants MAY carry role labels** ([decision 0012](../../../decisions/0012-role-labeled-endpoints-and-participants.md)). Each participants item is either the scalar qualified reference or a map `{ ref: <qualified-id>, role: <kebab-token> }` — required the moment the same entity participates in more than one role (`{ ref: contactius.contact, role: guardian }` and `{ ref: contactius.contact, role: ward }`). Two participants with the same reference and no distinguishing roles SHOULD be a lint warning.
